...
ssh azureuser@172.166.136.252  # worker node 2
...
sudo kubeadm init --pod-network-cidr=192.168.0.0/16 --apiserver-advertise-address=172.167.212.95 --node-name master
...
sudo kubeadm join 10.240.0.4:6443 --token kjpcbg.i0uw8o61wbg7vr6w \
        --discovery-token-ca-cert-hash sha256:faac6ed4fc880e27901321322f1f56e94d70bdbf8029c404079f09a7b23831db

# You can't run kubectl commands from worker nodes - only from the master node where the API server is running and kubeconfig is configured.
# Run kubectl get nodes from the MASTER node, not the worker node.
# On the master:
bashkubectl get nodes
kubectl get pods -A
# Worker nodes don't have the kubeconfig file (~/.kube/config) that tells kubectl how to connect to the API server. Only the master node has this configuration.
# If you want to run kubectl from worker nodes (optional), you'd need to copy the kubeconfig from master:
# bash# 
#On master, display the config
cat ~/.kube/config

# On worker, create the directory and paste the content
mkdir -p $HOME/.kube
nano $HOME/.kube/config  # paste the content and save
